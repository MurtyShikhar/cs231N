...
class KNearestNeighbor:
  ...
  def compute_distances_two_loops(self, X):
    ...
    for i in xrange(num_test):
      for j in xrange(num_train):
        ...
        #####################################################################
        vect_diff = X[i,:] - self.X_train[j,:]
        dists[i][j] = np.sqrt(np.sum(vect_diff**2))
  ...
  def compute_distances_no_loops(self, X):
    ...
    # X = num_test X D
    # X_train = num_train X D
    # (X - X_train)'(X - X_train)
    train_sum = np.sum(np.square(self.X_train), axis = 1) # num_train X 1
    test_sum = np.sum(np.square(X), axis = 1)  # num_test X num_test
    prod = np.dot(X, self.X_train.T) # num_test X num_train
    dists = np.sqrt(train_sum + test_sum.reshape(-1,1) -2*prod) 
    #########################################################################
    # TODO:                                                                 #
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      y_pred[i] = np.argmax(np.bincount(map(lambda key: self.y_train[key], closest_y)))
